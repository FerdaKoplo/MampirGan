...
namespace MampirGan_Automata__dan_Table_driven_.Fitur
{
    ...
    public class CartFeature
    {
        ...
        // dictionary untuk transisi state, menggantikan fungsionalitas if state (if (state == CartState.Active && event == CartEvent.ClearCart) return state = CartState.Empty;)) (table driven)
        ...
        public CartFeature()
        {
            // Jika ada data keranjang, maka state akan langsung berada pada state active
            ...
            // daftar command dengan menggunakan constructor dari CartRule
            // (key: input yang diketik)
            // (label: deskripsi yang diketik)
            // (event (berisi CartEvent) : event yang terjadi ketika user memilih menu)
            // (allowedstates: state yang diperlukan agar bisa menjalankan method)
            // (action: method yang akan dijalankan jika dipilih)
        ...
        public void Run()
        {
            ...
            while (running)
            {
                ...
                // Melakukan loop ke seluruh yang ada pada daftar cartCommands
                foreach (var cmd in cartCommands)
                {
                    //Jika state termasuk ke dalam AllowedStates, maka command akan diprint sebagai opsi
    ...
    public class CartService
    {
        ...
        private readonly string cartDataPath = "C:\\Users\\IVAN\\Documents\\Project_C#\\MampirGan-Automata-Table-driven-only-\\MampirGan(Automata, dan Table driven)\\MampirGan(Automata, dan Table driven)\\Jsons\\CartData.json";

        public CartService()
        {
            products = Jsonloader.LoadProducts("C:\\Users\\IVAN\\Documents\\Project_C#\\MampirGan-Automata-Table-driven-only-\\MampirGan(Automata, dan Table driven)\\MampirGan(Automata, dan Table driven)\\Jsons\\ProductDummy.json");
